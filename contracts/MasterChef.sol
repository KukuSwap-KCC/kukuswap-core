...
import "./KuKuToken.sol";

interface IMigratorChef {
    // Perform LP token migration from legacy UniswapV2 to kukuswap.
...
// MasterChef is the master of kuku. He can make kuku and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once kuku is sufficiently
...
contract MasterChef is Ownable {
    ...
    // The block number when kuku mining starts.
    ...
    // Update the given pool's kuku allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    // View function to see pending kukus on frontend.
    ...
    function updatePool(uint256 _pid) public {
        ...
        kuku.mint(devaddr, kukuReward.div(10));
        kuku.mint(address(this), kukuReward);
    ...
    // Deposit LP tokens to MasterChef for kuku allocation.
    ...
    // Safe kuku transfer function, just in case if rounding error causes pool to not have enough kukus.
